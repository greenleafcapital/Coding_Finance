...
colnames(fv_table_both) <- paste(c("Year", "Tom's inital value", "Tom's Deposit", "Tom's Interest", "Tom's Ending value", "Jerry's inital value", "Jerry's Deposit", "Jerry's Interest", "Jerry's Ending value"))

options(digits = 2)

fv_table_both %>%
  gather(`Tom's inital value`:`Jerry's Ending value`, key = Name, value = save, factor_key = TRUE) %>%
  ...
  datatable(caption = "Comparing the Savings and investment growth \n for Tom
            and Jerry", rownames = FALSE,options = list(pageLength = 10,
                          autoWidth = TRUE,
                          scrollX = TRUE)) %>%
  formatRound(columns = c("Tom's inital value", "Tom's Deposit", "Tom's Interest", "Tom's Ending value", "Jerry's inital value", "Jerry's Deposit", "Jerry's Interest", "Jerry's Ending value"), digits = 2)
