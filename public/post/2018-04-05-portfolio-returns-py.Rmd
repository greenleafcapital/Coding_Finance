...
In the example given in the R post we calculate the portfolio returns using the tidy dataframe. We can also calculate the returns using a tidy method in Python. To do that we need to reshape our returns dataframe and create a new weights table. We will then join the two and calculate the portfolio returns.
...
So we now have the portfolio returns calculated in tidy format as well. We can see that the results are same since the difference between the two is 0.
...
- To transform the data into tudy format and calculate the returns
